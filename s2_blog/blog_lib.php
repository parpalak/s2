...
function s2_blog_show_comments ($id)
{
	global $s2_db, $session_id, $lang_admin, $lang_s2_blog;
	...
	($hook = s2_hook('fn_blog_show_comments_pre_get_cmnts_qr')) ? eval($hook) : null;
	$result = $s2_db->query_build($query) or error(__FILE__, __LINE__);
	if (!$s2_db->num_rows($result))
		return '<h2>'.$lang_s2_blog['Blog hidden comments'].'</h2><div class="info-box"><p>'.$lang_admin['No comments'].'</p></div>';
	...
	($hook = s2_hook('fn_blog_show_comments_pre_get_perm_qr')) ? eval($hook) : null;
	$result2 = $s2_db->query_build($query) or error(__FILE__, __LINE__);
	$show_hidden = $s2_db->num_rows($result2) == 1 && $s2_db->result($result2, 0, 0);
	...
	while ($row = $s2_db->fetch_assoc($result))
	{
		...
		($hook = s2_hook('fn_blog_show_comments_pre_class_merge')) ? eval($hook) : null;
		$class = !empty($class) ? ' class="'.implode(' ', $class).'"' : '';

		// Preparing row buttons
		$buttons = array();
		$buttons['edit'] = '<img src="i/ce.png" alt="'.$lang_admin['Edit'].'" width="16" height="16" onclick="return EditBlogComment('.$row['id'].');" />';
		$buttons['hide'] =  $row['shown'] ?
			'<img src="i/cd.png" alt="'.$lang_admin['Hide'].'" width="16" height="16" onclick="return DoAction(\'hide_blog_comment\', '.$row['id'].', \'comm_div\');" />' :
			'<img src="i/ca.png" alt="'.$lang_admin['Show'].'" width="16" height="16" onclick="return DoAction(\'hide_blog_comment\', '.$row['id'].', \'comm_div\')" />' ;

		$buttons['mark'] = $row['good'] ?
			'<img src="i/thumb_down.png" alt="'.$lang_admin['Unmark comment'].'" width="16" height="16" onclick="return DoAction(\'mark_blog_comment\', '.$row['id'].', \'comm_div\');" />' :
			'<img src="i/thumb_up.png" alt="'.$lang_admin['Mark comment'].'" width="16" height="16" onclick="return DoAction(\'mark_blog_comment\', '.$row['id'].', \'comm_div\');" />';

		$buttons['delete'] = '<img src="i/cross.png" alt="'.$lang_admin['Delete'].'" width="16" height="16" onclick="return DeleteBlogComment('.$row['id'].');" />';

		($hook = s2_hook('fn_blog_show_comments_pre_buttons_merge')) ? eval($hook) : null;
		$buttons = '<nobr>'.implode('', $buttons).'</nobr>';
		...
		if ($show_hidden)
		{
			...
			($hook = s2_hook('fn_blog_show_comments_pre_email_status_merge')) ? eval($hook) : null;
			$email_status_merged = !empty($email_status) ? ' ('.implode(', ', $email_status).')' : '';

			($hook = s2_hook('fn_blog_show_comments_pre_email_merge')) ? eval($hook) : null;
		...
		($hook = s2_hook('fn_blog_show_comments_pre_table_row_merge')) ? eval($hook) : null;
	...
	if ($id)
	{
		...
		($hook = s2_hook('fn_blog_show_comments_pre_get_title_query')) ? eval($hook) : null;
		$result = $s2_db->query_build($query) or error(__FILE__, __LINE__);
		list($table_title) = $s2_db->fetch_row($result);

		($hook = s2_hook('fn_blog_show_comments_pre_title_merge')) ? eval($hook) : null;
		$table_title = sprintf($lang_s2_blog['Blog comments'], $table_title);
	}
	else
		$table_title = $lang_s2_blog['Blog hidden comments'];
	...
	($hook = s2_hook('fn_blog_show_comments_end')) ? eval($hook) : null;
	return $output;
}
