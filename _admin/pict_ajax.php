...
if ($action == 'load_tree') {
    ...
    if (isset($_POST['dir'])) {
        // TODO validate that $path starts from / or is empty
        $path = $_POST['dir'];
    ...
    if (!isset($_FILES['pictures']))
        ...
        foreach ($_FILES['pictures']['name'] as $i => $filename) {
            ...
            if (($_POST['create_dir'] ?? false) && !is_dir(S2_IMG_PATH . $path)) {
                if (!mkdir(S2_IMG_PATH . $path, 0777, true) && !is_dir(S2_IMG_PATH . $path)) {
                    throw new \RuntimeException(sprintf('Directory "%s" was not created', S2_IMG_PATH . $path));
                }
    ...
    if (isset($_POST['ajax'])) {
        if (!empty($errors)) {
            header('Content-Type: application/json; charset=utf-8');
            echo s2_json_encode(['errors' => $errors]);

        } else {
            header('Content-Type: application/json; charset=utf-8');
            $result = [
                'status'    => 'ok',
                'file_path' => S2_PATH . '/' . S2_IMG_DIR . $path . '/' . $filename
            ];
            if (($_POST['return_image_info'] ?? false) && function_exists('getimagesize')) {
                $result['image_info'] = getimagesize($uploadfile);
            }
            echo s2_json_encode($result);
        }
    } else {
        if (!empty($errors)) {
            echo sprintf($lang_pictures['Upload failed'], implode('<br />', $errors));
        }
    }
}
