...
function s2_attachment_list1 ($id, $parent_path, $config)
{
	...
	$query = array(
		'SELECT'	=> 'f.name, f.filename, f.time AS upload_time, f.size, a.title, a.create_time, a.modify_time, a.excerpt, a.pagetext, a.id, ('.$raw_query1.') IS NOT NULL AS children_exist, a.url',
		...
		'WHERE'		=> 'a.parent_id = '.$id.' AND a.published = 1 AND f.is_picture = 0',
		'ORDER BY'	=> 'time'
	);
	...
	while ($row = $s2_db->fetch_assoc($result))
	{
		...
		{
			if ($conf_name == 'file')
				$item = '<a href="'.S2_PATH.'/'.S2_IMG_DIR.'/'.date('Y', $row['upload_time']).'/'.$row['id'].'/'.$row['filename'].'">'.s2_htmlencode($row['name'] ? $row['name'] : $row['filename']).'</a>';
			elseif ($conf_name == 'title')
				$item = '<a href="'.S2_PATH.S2_URL_PREFIX.$parent_path.'/'.urlencode($row['url']).($row['children_exist'] ? '/' : '').'">'.$row['title'].'</a>';
			else
				$item = $row[$conf_name];
			$tr .= '<td>'.$item.'</td>';
		}
...
function s2_attachment_list ($id, $parent_path, $current_path, $config, $page_limit)
{
	global $s2_db, $lang_s2_attachment;
