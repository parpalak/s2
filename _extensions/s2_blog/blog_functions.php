...
function s2_blog_get_post ($year, $month, $day, $url, $template)
{
	...
	$link_nav = array('up' => S2_BLOG_PATH . date('Y/m/d/', $start_time));

	if (strpos($template, '<!-- s2_back_forward -->') !== false || strpos($template, '<!-- s2_navigation_link -->') !== false)
	{
		$query = array(
			'SELECT'   => 'create_time, title, url',
			'FROM'     => 's2_blog_posts AS p',
			'WHERE'    => 'create_time < ' . $row['create_time'] . ' AND published = 1',
			'ORDER BY' => 'create_time DESC',
			'LIMIT'    => '1',
		);
		($hook = s2_hook('fn_s2_blog_get_post_pre_get_prev_post_qr')) ? eval($hook) : null;
		$result = $s2_db->query_build($query) or error(__FILE__, __LINE__);
		$prev_post = $s2_db->fetch_assoc($result);
		...
		($hook = s2_hook('fn_s2_blog_get_post_pre_get_next_post_qr')) ? eval($hook) : null;
		$result = $s2_db->query_build($query) or error(__FILE__, __LINE__);
		$next_post = $s2_db->fetch_assoc($result);

		$link_nav = array();
		if ($prev_post)
		{
			$link_nav['prev'] = S2_BLOG_PATH . date('Y/m/d/', $prev_post['create_time']) . urlencode($prev_post['url']);
			$html['prev'] = '<li class="back"><span class="arrow">&larr;</span> <a href="' . $link_nav['prev'] . '">' . s2_htmlencode($prev_post['title']) . '</a></li>';
		}
		else
			$html['prev'] = '<li class="back empty"><span class="arrow">&larr;</span> </li>';

		if ($next_post)
		{
			$link_nav['next'] = S2_BLOG_PATH . date('Y/m/d/', $next_post['create_time']) . urlencode($next_post['url']);
			$html['next'] = '<li class="forward"><span class="arrow">&rarr;</span> <a href="' . $link_nav['next'] . '">' . s2_htmlencode($next_post['title']) . '</a></li>';
		}
		else
			$html['next'] = '<li class="forward empty"><span class="arrow">&rarr;</span> </li>';

		$back_forward = '<ul class="back_forward">'. $html['prev'] . $html['next'] . '</ul>';
	}
	...
	$return = array(
		...
		'id'              => $post_id,
		'link_navigation' => $link_nav
	);

	if (isset($back_forward))
		$return['back_forward'] = $back_forward;
