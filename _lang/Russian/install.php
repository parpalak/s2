...
'Part1 intro'				=>	'Введите информацию о базе данных, которую будет использовать движок S2. Вы должны узнать все параметры, запрашиваемые ниже.',
...
'Database type info'		=>	'S2 может работать с базами данных MySQL, PostgreSQL и SQLite. Если PHP не поддерживает какую-нибудь базу данных, она не появится в выпадающем списке ниже. Более полная информация о возможности использования конкретных версий баз данных есть в документации.',
...
'MySQL InnoDB info'			=>	'Возможно, сервер MySQL может поддерживать таблицы <a href="http://dev.mysql.com/doc/refman/5.0/en/innodb-overview.html">InnoDB</a>. Если вы планируете сделать большой сайт, обратите внимание на таблицы этого типа. Но если вы сомневаетесь, лучше не использовать их.',
'Database server info'		=>	'Введите адрес сервера базы данных (например, <em>localhost</em>, <em>mysql1.example.com</em> или <em>208.77.188.166</em>). Вы можете указать и порт, если база данных работает с портом, отличающимся от порта по умолчанию (например, <em>localhost:3580</em>). For SQLite support, just enter anything or leave it at \'localhost\'.',
'Database name info'		=>	'Укажите название базы данных, в которую нужно установить S2. Эта база данных должна существовать. For SQLite, this is the relative path to the database file. If the SQLite database file does not exist, S2 will attempt to create it.',
'Database username info'	=>	'Введите имя пользователя и пароль, необходимые для подключения к базе данных. Ignore for SQLite.',
'Table prefix info'			=>	'Вы можете указать строку, с которой будут начинаться все создаваемые таблицы (например, <em>test_</em>). В одну и ту же базу данных можно будет установить несколько копий S2 с разными префиксами таблиц.',
...
'Part3 intro'				=>	'Укажите требуемую информацию о сайте. Обратите особое внимание на поле «URL сайта».',
'Base URL'					=>	'URL сайта',
'Base URL info'				=>	'Сюда необходимо ввести адрес будущего сайта, без завершающего слеша (например, <em>http://example.com</em> или <em>http://example.com/~myuser</em>). Вы обязательно должны указать корректное значение, иначе сайт заработает с ошибками! Заметьте, сразу после загрузки в этом поле содержится всего лишь догадка о том, каким должен быть URL сайта.',
...
'No write info 1'			=>	'<strong>Внимание!</strong> Чтобы завершить установку, нажмите на эту кнопку и скачайте файл config.php. Затем загрузите этот файл на сервер в ту директорию, в которую вы закачивали файлы S2.',
...
'No cache write'			=>	'<strong>Отсутствуют права на запись в папку cache!</strong> Во время работы S2 пишет файлы в директорию <em>cache</em>. Скрипты PHP должны обладать правами для создания файлов в этой директории. Измените разрешения на доступ к директории cache командой chmod. Если не знаете, какие разрешения устанавливать, установите 0777.',
'No pictures write'			=>	'<strong>Отсутствуют права на запись в папку pictures!</strong> S2 сохраняет загруженные картинки и другие файлы в директорию <em>pictures</em>. Если вы планируете загружать картинки, убедитесь, что скрипты PHP обладают правами для создания файлов в этой директории. При необходимости измените разрешения на доступ к директории pictures командой chmod. Если не знаете, какие разрешения устанавливать, установите 0777.',
