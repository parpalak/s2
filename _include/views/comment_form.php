...
?>
<h2 class="comment form"><?php echo $lang_common['Post a comment']; ?></h2>
<form method="post" name="post_comment" action="<?php echo $action?>">
	...
	<p class="input email">
		...
		<label for="show_email" title="<?php echo $lang_common['Show email label title']; ?>"><input type="checkbox" id="show_email" name="show_email" <?php if ($show_email) echo 'checked="checked" '; ?>/><?php echo $lang_common['Show email label']; ?></label><br />
		<label for="subscribed" title="<?php echo $lang_common['Subscribe label title']; ?>"><input type="checkbox" id="subscribed" name="subscribed" <?php if ($subscribed) echo 'checked="checked" '; ?>/><?php echo $lang_common['Subscribe label']; ?></label>
	</p>
	<p class="input text">
		<?php echo $lang_common['Your comment']; ?><br />
		<textarea cols="50" rows="10" name="text"><?php echo s2_htmlencode($text); ?></textarea>
		<br />
		<small class="comment-syntax"><?php ($hook = s2_hook('v_comment_form_pre_syntax_info')) ? eval($hook) : null; echo $lang_common['Comment syntax info']; ?></small>
	</p>
	<p id="qsp">
